...
class ConfigInfo:
    ...
    def __init_output_config(self, monitors, logical_monitors):
        ...
        for lm in logical_monitors:
            ...
            for m in lm[5]:
                ...
                # Store effective display dimensions (accounting for current rotation)
                if lm[3] in [1, 3]:  # left=1, right=3: 90° rotations swap w and h
                    conf['w'] = h
                    conf['h'] = w
                else:
                    conf['w'] = w
    ...
    def output_set_mode_by_res(self, output, res):
        ...
        if new_mode:
            conf['mode-info'] = new_mode
            conf['res'] = res
            # Preserve effective dimensions (accounting for current rotation)
            if conf['trans'] in [1, 3]:  # 90° rotation: effective dims are swapped
                conf['w'] = new_mode[2]
                conf['h'] = new_mode[1]
            else:
                conf['w'] = new_mode[1]
